<template>
  <div class="indices-management">
    <div class="toolbar">
      <el-button type="primary" @click="showAddDialog">添加指数</el-button>
    </div>
...
export default {
  ...
  setup() {
    ...
    const resetForm = () => {
      ...
      Object.keys(form).forEach(key => {
        form[key] = key === 'current_point' || key === 'change_percent' || 
                   key === 'support_point' || key === 'pressure_point' || 
                   key === 'progress' ? null : ''
      })
    }
